...
public final class MapFileWriter {
			...
			for (byte zoomlevel = minZoomCurrentInterval; zoomlevel <= maxZoomCurrentInterval; zoomlevel++) {
				...
				if (pois != null) {
					...
					for (TDNode poi : pois) {
						...
						processPOI(poi, tags, currentTileLat, currentTileLon, configuration.isDebugStrings(),
								poiDataBuffer);
					}
				...
				try {
					...
					for (Future<WayPreprocessingResult> wprFuture : futures) {
						...
						if (wpr != null) {
							...
							processWay(wpr, way, tags, strings, currentTileLat, currentTileLon, wayBuffer);
	...
	static void processPOI(TDNode poi, short[] tags, int currentTileLat, int currentTileLon, boolean debugStrings,
	...
	static void processWay(WayPreprocessingResult wpr, TDWay way, short[] tags, HashMap<String, Integer> strings,
	...
	private static class WayPreprocessingCallable implements Callable<WayPreprocessingResult> {
		...
		public WayPreprocessingResult call() {
			...
			if (originalGeometry instanceof Polygon || this.way.isPolygon()) {
				...
				if (area < minArea) {
					droppedPoly++;
					// System.out.println("way " + this.way.getName() + " " + area);
					return null;
				}
