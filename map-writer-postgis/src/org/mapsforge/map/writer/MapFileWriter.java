...
import org.mapsforge.map.writer.model.OSMTagMapping;
...
import de.sfb.tilemap.writer.MapWriter;
...
//	private static final int JTS_GEOMETRY_CACHE_SIZE = 50000;
	...
	static volatile int droppedWays = 0;
	static volatile int droppedPoly = 0;
	...
	public static void writeFile(MapWriterConfiguration configuration,
		...
		int amountOfZoomIntervals = dataProcessor.getZoomIntervalConfiguration()
		...
		final LoadingCache<TDWay, Geometry> jtsGeometryCache = null;
		...
		LOGGER.info("Finished writing file. " + droppedWays + " " + droppedPoly);
	}
	...
	static int writeHeaderBuffer(final MapWriterConfiguration configuration,
		...
		// AMOUNT POI TAGS
		containerHeaderBuffer.putShort((short) configuration.getTagMapping().getOptimizedPoiIds()
				.size());
		...
		// AMOUNT OF WAY TAGS
		// containerHeaderBuffer.putShort((short) configuration.getTagMapping().getOptimizedWayIds()
		// .size());
		...
		// }
		// AMOUNT OF WAY TAGS
		containerHeaderBuffer.putShort((short) configuration.getTagMapping().getWayTags()
				.size());

		// WAY TAGS
		for (OSMTag tag : configuration.getTagMapping().getWayTags()) {
			// System.out.println(tag.tagKey());
			writeUTF8(tag.tagKey(), containerHeaderBuffer);
		}

		// AMOUNT OF ZOOM INTERVALS
		int numberOfZoomIntervals = dataProcessor.getZoomIntervalConfiguration()
				.getNumberOfZoomIntervals();
	...
	private static long writeSubfile(final long startPositionSubfile, final int zoomIntervalIndex,
		...
		final TileCoordinate upperLeft = dataStore.getTileGridLayout(zoomIntervalIndex)
				.getUpperLeft();
		...
		final byte baseZoomCurrentInterval = dataStore.getZoomIntervalConfiguration()
				.getBaseZoom(zoomIntervalIndex);
		...
		final int indexBufferSize = tileAmountInBytes
				+ (configuration.isDebugStrings() ? DEBUG_INDEX_START_STRING.getBytes(UTF8_CHARSET).length
						: 0);
		...
		for (int tileY = upperLeft.getY(); tileY < upperLeft.getY() + lengthY; tileY++) {
			for (int tileX = upperLeft.getX(); tileX < upperLeft.getX() + lengthX; tileX++) {
				TileCoordinate tileCoordinate = new TileCoordinate(tileX, tileY,
						baseZoomCurrentInterval);

				processIndexEntry(tileCoordinate, indexBuffer, currentSubfileOffset);
				processTile(configuration, tileCoordinate, dataStore, jtsGeometryCache,
						zoomIntervalIndex, tileBuffer,
	...
	private static void writeIndex(ByteBuffer indexBuffer, long startPositionSubfile,
			long subFileSize,
			RandomAccessFile randomAccessFile) throws IOException {
	...
	private static void processTile(MapWriterConfiguration configuration,
			...
			int zoomIntervalIndex, ByteBuffer tileBuffer, ByteBuffer poiDataBuffer,
			ByteBuffer wayDataBuffer,
		...
		class SortNodeByTags implements Comparator<TDNode> {

			@Override
			public int compare(TDNode node1, TDNode node2) {
				if (node1 == null)
					return -1;
				if (node2 == null)
					return 1;

				if (node1.getTags() == null)
					return -1;
				if (node2.getTags() == null)
					return 1;
				int len1 = node1.getTags().length;
				int len2 = node2.getTags().length;
				if (len1 != len2)
					return (len1 < len2) ? -1 : 1;

				for (int i = 0; i < len1; i++)
					if (node1.getTags()[i] != node2.getTags()[i])
						return (node1.getTags()[i] < node2.getTags()[i]) ? -1 : 1;

				return 0;
			}
		}
		SortWayByTags waySort = new SortWayByTags();
		SortNodeByTags nodeSort = new SortNodeByTags();
		...
		final int currentTileLat = Coordinates.degreesToMicrodegrees(MercatorProjection
				.tileYToLatitude(tileCoordinate.getY(), tileCoordinate.getZoomlevel()));
		final int currentTileLon = Coordinates.degreesToMicrodegrees(MercatorProjection
				.tileXToLongitude(tileCoordinate.getX(), tileCoordinate.getZoomlevel()));

		final byte minZoomCurrentInterval = dataProcessor.getZoomIntervalConfiguration()
				.getMinZoom(zoomIntervalIndex);
		final byte maxZoomCurrentInterval = dataProcessor.getZoomIntervalConfiguration()
				.getMaxZoom(zoomIntervalIndex);
		...
		for (byte zoomlevel = minZoomCurrentInterval; zoomlevel <= maxZoomCurrentInterval; zoomlevel++) {
			...
			if (pois != null) {
				...
				Collections.sort(pois, nodeSort);

				for (TDNode poi : pois) {
					short[] tags = poi.getTags();

					if (prevTags == null || (prevTags.length != tags.length))
						prevTags = tags;
					else {
						boolean same = true;

						for (int i = 0, n = tags.length; i < n; i++) {
							if (prevTags[i] != tags[i]) {
								same = false;
								break;
							}
						}
					...
					processPOI(poi, tags, currentTileLat, currentTileLon,
							configuration.isDebugStrings(),
							poiDataBuffer);
				}
		...
		List<TDWay> ways = MapWriter.getWaysForTile(tileCoordinate);
		...
		for (byte zoomlevel = minZoomCurrentInterval; zoomlevel <= maxZoomCurrentInterval; zoomlevel++) {
			...
			for (TDWay way : ways) {
				...
				String str = way.getName();
				if (str != null && !str.isEmpty() && !strings.containsKey(str))
					strings.put(str, null);

				str = way.getRef();
				if (str != null && !str.isEmpty() && !strings.containsKey(str))
					strings.put(str, null);

				str = way.getHouseNumber();
				if (str != null && !str.isEmpty() && !strings.containsKey(str))
					strings.put(str, null);
			}
		}
		...
		for (Entry<String, Integer> entry : strings.entrySet()) {
			entry.setValue(new Integer(wayBuffer.position()));
			writeUTF8(entry.getKey(), wayBuffer);
		...
		for (byte zoomlevel = minZoomCurrentInterval; zoomlevel <= maxZoomCurrentInterval; zoomlevel++) {
			...
			// List<TDWay> ways = waysByZoomlevel.get(Byte.valueOf(zoomlevel));
			// System.out.println(">> ways" + ways == null ? "null" : ways.size());
			if (ways == null)
				continue;

			List<WayPreprocessingCallable> callables = new ArrayList<MapFileWriter.WayPreprocessingCallable>();

			Collections.sort(ways, waySort);

			for (TDWay way : ways) {
				if (!way.isInvalid() && way.getTags() != null && way.getTags().length > 0) {
			...
			try {
				...
				for (Future<WayPreprocessingResult> wprFuture : futures) {
					...
					if (wpr != null) {
						...
						processWay(wpr, way, tags, strings, currentTileLat, currentTileLon,
								wayBuffer);
			...
			break; // FIXME only ways for base zoomlevel for now
		}
	...
	static void processPOI(TDNode poi, short[] tags, int currentTileLat, int currentTileLon,
		...
		if (debugStrings) {
			...
			appendWhitespace(DEBUG_BLOCK_SIZE - sb.toString().getBytes(UTF8_CHARSET).length,
					poiBuffer);
		}
		...
		short tagAmount = tags == null ? 0 : (short) tags.length;

		byte layer = poi.getLayer();
		// make sure layer is in [0,10]
		layer = layer < 0 ? 0 : layer > 10 ? 10 : layer;

		byte infoByte = (byte) (poi.getLayer() << BYTES_INT | tagAmount);

		poiBuffer.put(infoByte);
	...
	static void processWay(WayPreprocessingResult wpr, TDWay way, short[] tags,
		...
		byte infoByte = (byte) (way.getLayer() << BYTES_INT | tagAmount);

		wayBuffer.put(infoByte);

		// write tag ids
		if (tags != null) {
			for (short tagID : tags) {
				wayBuffer.put(Serializer.getVariableByteUnsigned(tagID)); // mappedWayTagID(tagID)));
			}
		}
		...
		if (str != null && !str.isEmpty()) {
			wayBuffer.put(Serializer.getVariableByteUnsigned(strings.get(str).intValue()));
		}
		str = way.getHouseNumber();
		if (str != null && !str.isEmpty()) {
			wayBuffer.put(Serializer.getVariableByteUnsigned(strings.get(str).intValue()));
		...
		if (wpr.getLabelPosition() != null) {
			...
			wayBuffer.put(Serializer.getVariableByteSigned(wpr.getLabelPosition().latitudeE6
					- firstWayStartLat));
			wayBuffer.put(Serializer.getVariableByteSigned(wpr.getLabelPosition().longitudeE6
					- firstWayStartLon));
		}
		...
		for (WayDataBlock wayDataBlock : wpr.getWayDataBlocks()) {
			...
			if (wayDataBlock.getInnerWays() != null && !wayDataBlock.getInnerWays().isEmpty()) {
				...
				// inner ways
				wayBuffer.put(Serializer.getVariableByteUnsigned(1 + wayDataBlock.getInnerWays()
						.size()));
			} else {
	...
	static void writeWayNodes(List<Integer> waynodes, int currentTileLat, int currentTileLon,
			ByteBuffer buffer) {
	...
	private static int mappedWayTagID(short original) {
		return OSMTagMapping.getInstance().getOptimizedWayIds().get(Short.valueOf(original))
				.intValue();
	...
	// // make sure layer is in [0,10]
	// layer = layer < 0 ? 0 : layer > 10 ? 10 : layer;
	// short tagAmount = node.getTags() == null ? 0 : (short)
	// node.getTags().length;
	//
	// return (byte) (layer << BYTES_INT | tagAmount);
	// }
	//
	// static byte infoByteLayerAndTagAmount(TDWay way, short tagAmount) {
	// byte layer = way.getLayer();
	// // make sure layer is in [0,10]
	// layer = layer < 0 ? 0 : layer > 10 ? 10 : layer;
	//
	...
	private static class WayPreprocessingResult {
		...
		WayPreprocessingResult(TDWay way, List<WayDataBlock> wayDataBlocks, GeoPoint labelPosition,
				short subtileMask) {
			super();
			this.way = way;
	...
	private static class WayPreprocessingCallable implements Callable<WayPreprocessingResult> {
		...
		public WayPreprocessingResult call() {
			...
			Geometry originalGeometry = this.way.getGeometry();
			this.way.setGeometry(null);
			// try {
			...
			// LineString l = (LineString) processedGeometry;
			// if (l.getPointN(0).equals(l.getEndPoint())) {
			// //System.out.println("reverse");
			//
			// processedGeometry.normalize();
			// }
			// }

			// int x = this.tile.getX() * Tile.TILE_SIZE + Tile.TILE_SIZE / 2;
			// int y = this.tile.getY() * Tile.TILE_SIZE + Tile.TILE_SIZE / 2;
			// byte z = this.tile.getZoomlevel();
			//
			// if (originalGeometry instanceof Polygon || this.way.isPolygon()) {
			// int min = 3;
			// double lo1 = MercatorProjection.pixelXToLongitude(x, z);
			// double lo2 = MercatorProjection.pixelXToLongitude(x + min, z);
			// double la1 = MercatorProjection.pixelYToLatitude(y, z);
			// double la2 = MercatorProjection.pixelYToLatitude(y + min, z);
			//
			// double area = originalGeometry.getEnvelope().getArea();
			//
			// double minArea = Math.abs((lo1 - lo2) * (la1 - la2));
			//
			// if (area < minArea) {
			// droppedPoly++;
			// // System.out.println("way " + this.way.getName() + " " +
			// // area);
			// return null;
			// }
			// } else if (!this.way.isCoastline()) {
			// int min = 1;
			// double lo1 = MercatorProjection.pixelXToLongitude(x, z);
			// double lo2 = MercatorProjection.pixelXToLongitude(x + min, z);
			// double la1 = MercatorProjection.pixelYToLatitude(y, z);
			// double la2 = MercatorProjection.pixelYToLatitude(y + min, z);
			//
			// double length = originalGeometry.getEnvelope().getLength();
			// if (length < Math.sqrt((lo1 - lo2) * (lo1 - lo2) + (la1 - la2) * (la1 - la2))) {
			// // System.out.println("way " + this.way.getName() + " " +
			// // area);
			// droppedWays++;
			// return null;
